...
from src.utils import match_events, compute_return_levels
...
for key, station in STATIONS.items():
    ...
    retro.columns = ["discharge"]; retro.index = to_naive(retro.index)
    ...
    # GEOGloWS corrected
    corr = pd.read_parquet(DATA_DIR / f"geoglows_retro_daily_corrected_{rid}.parquet")
    corr.columns = ["discharge"]; corr.index = to_naive(corr.index)
