...
for key, station in STATIONS.items():
    ...
    orig = pd.read_parquet(DATA_DIR / f"geoglows_retro_daily_{rid}.parquet")
    orig.columns = ["discharge"]; orig.index = orig.index.tz_localize(None)
    rp_orig_emp = compute_return_levels(orig["discharge"], RETURN_PERIODS)

    corr_path = DATA_DIR / f"geoglows_retro_daily_corrected_{rid}.parquet"
    corr = pd.read_parquet(corr_path)
    corr.columns = ["discharge"]
    if corr.index.tz is not None:
        corr.index = corr.index.tz_localize(None)
    rp_corr_emp = compute_return_levels(corr["discharge"], RETURN_PERIODS)
...
Choosing thresholds from GEOGloWS as-published would produce *fewer* triggers than GloFAS thresholds (higher bar). Choosing them from the SFDC-corrected series would produce *more* triggers in 3 of 4 cases (lower bar). Neither shift necessarily improves real-world detection — that depends on whether the events the model "sees" are the ones DHM actually observed. The next chapter compares time-series and seasonal patterns across the sources; @sec-event-detection then runs the formal validation against DHM crossings.
